            ...
            Exchange::OCDM_RESULT CreateContext(const string& keySystem, bool cleanOnDestroy, Exchange::IOCDMContext*& context) override
            {
                ...
                if (system != nullptr) {
                    ...
                    if (systemExt != nullptr) {
                        ...
                        context = Core::Service<ContextImplementation>::Create<ContextImplementation>(this, keySystem, cleanOnDestroy);

                        _adminLock.Lock();
                        _contextList.push_front(dynamic_cast<ContextImplementation*>(context));
                        TRACE_L1("context created: %p, number of context instances %d", context, _contextList.size());
                        _adminLock.Unlock();
                    }
                } else {
